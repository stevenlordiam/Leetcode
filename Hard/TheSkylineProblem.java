...
Notice that "key points" are either the left or right edges of the buildings. Therefore, we first obtain both the edges of all the N buildings, and store the 2N edges in a sorted array. 
Maintain a max-heap of building heights while scanning through the edge array: If the current edge is a left edge, then add the height of its associated building to the max-heap; 
if the edge is a right one, remove the associated height from the heap. Then we take the top value of the heap (yi) as the maximum height at the current edge position (xi). 
Now (xi, yi) is a potential key point. If yi is the same as the height of the last key point in the result list, it means that this key point is not a REAL key point, 
but rather a horizontal continuation of the last point, so it should be discarded; otherwise, we add (xi,yi) to the result list because it is a real key point. 
...
For C++ programmers: As appealing as the solution may sound, it is not particularly C++ friendly. As of yet, C++ standard still does not support removal of a non-top element from a heap in a portable way. 
If you are to try this solution in C++, you may need to implement your own heap structure or even resort to other libraries that supports the removal operation.

2. Divide and Conquer:
Divide the list of buildings into two halves. Recursively get the skyline of both halves, and then merge the two skylines in O(n) time (see if you can figure out this part yourself). 
According to Master's Theorem, this solution takes O(NlogN) time as well, the same as merge sort.

https://leetcode.com/discuss/37420/clean-java-solution-using-two-priorityqueues
https://leetcode.com/discuss/37427/java-ac-solution-with-comments
https://leetcode.com/discuss/37455/my-java-solution-520ms-with-bst
https://leetcode.com/discuss/37494/my-java-solution-using-a-max-queue
https://leetcode.com/discuss/37519/java-faster-solution-with-heap-instead-java-priority-queue
https://leetcode.com/discuss/39533/share-my-java-solution-using-hashmap-and-priorityqueue
*/